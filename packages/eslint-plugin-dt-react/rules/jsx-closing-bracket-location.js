...
module.exports = {
  meta: {
    docs: {
      ...
      recommended: false,
      url: 'https://github.com/DTStack/Code-Style-Guide/tree/master/react#alignment',
    },
    fixable: 'code',
  ...
  create(context) {
    ...
    function getIndentation(tokens, expectedLocation, correctColumn) {
      ...
      switch (expectedLocation) {
        case 'props-aligned':
          indentation = /^\s*/.exec(
            context.getSourceCode().lines[tokens.lastProp.firstLine - 1]
          )[0];
          break;
        case 'tag-aligned':
        case 'line-aligned':
          indentation = /^\s*/.exec(
            context.getSourceCode().lines[tokens.opening.line - 1]
          )[0];
          break;
        default:
          indentation = '';
