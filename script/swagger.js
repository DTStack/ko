...
const parseRenderString = (str) => {
    ...
    parseArr.forEach((item) => {
        if (tagRegex.test(item)) {
            ...
            try {
                let method = methodRegex.test(item) ? item.match(methodRegex)[0].trim() : '';
                let url = urlRegex.test(item) ? item.match(urlRegex)[0].trim() : '';
                let key = keyRegex.test(item) ? item.match(keyRegex)[0].trim() : '';
                let comment = commentRegex.test(item) ? item.match(commentRegex)[0].trim() : '';
...
function handleFileData(path ,data) {
    ...
    let interfaceData = handleResolveArray(fileResult, data); // 将已存在的文件数据内容和需要新添加的数据内容合并
    let renderString = renderMustache(interfaceData);
...
function writerFile(filePath, renderString) {
    fs.writeFile(filePath, renderString, function (err) {
        if (err)
            Log(Colors.red('生成api文件操作失败'));
        else
            Log(Colors.green(`生成api文件操作成功,生成目录: ${filePath} `));
    });
}
...
function renderMustache(interfaceData) {
    ...
    let renderString = Mustache.render(temp, { result: interfaceData });
    return renderString;
}
...
function sortByTags(paths, result) {
    ...
    Object.keys(paths).map((reqApi) => {
        ...
        let object = cloneResult.find(o => o.name == info.tags[0]);
        !/(?<=\{)[^\},]*/.test(reqApi) && object.data.push({ ...info, path: reqApi, method, });
    });
    return cloneResult;
}
        ...
        .then(api => {
            get({}, encodeURI(api)).then((res) => {
                ...
                if (folderExist) {
                    ...
                    if (fileExist) {
                        //若文件存在,则处理内容
                        const fileResult = handleFileData(Path.join(filePath, FILE_NAME), interfaceData);
